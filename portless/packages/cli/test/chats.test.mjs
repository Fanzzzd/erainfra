  ...
  writeFileSync(
    ...
    [
      ...
      JSON.stringify({
        ...
        message: {
          role: "user",
          content: [{ tool_use_id: "t1", type: "tool_result", content: "file contents" }],
        },
      }),
      JSON.stringify({
        type: "user",
      ...
      JSON.stringify({
        ...
        message: { role: "user", content: "thanks, deploy it" },
      }),
    ].join("\n"),
  );
  try {
  ...
  writeFileSync(
    ...
    [
      ...
      JSON.stringify({
        ...
        type: "turn_context",
        payload: { model: "gpt-5.5", cwd: "/Users/me/code/blog" },
      }),
      JSON.stringify({
        ...
        payload: {
          type: "message",
          role: "user",
          content: [{ type: "input_text", text: "center a div please" }],
        },
      }),
      JSON.stringify({
        ...
        payload: {
          type: "message",
          role: "assistant",
          content: [{ type: "output_text", text: "Use place-items: center." }],
        },
      }),
      JSON.stringify({
        timestamp: "2026-07-01T10:00:10Z",
        type: "event_msg",
        payload: { type: "user_message", message: "center a div please" },
      }),
    ].join("\n"),
  );
  try {
    ...
    assert.deepEqual(
      messages.map((m) => m.role),
      ["user", "assistant"],
    ); // event_msg duplicate NOT double-counted
