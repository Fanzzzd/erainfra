...
export class PortAllocator {
  ...
  alloc(node: string, key: string): number {
    const existing = this.d
      .prepare("SELECT port FROM ports WHERE node = ? AND key = ?")
      .get(node, key) as { port: number } | undefined;
    if (existing) return Number(existing.port);
    const used = new Set(
      (
        this.d.prepare("SELECT port FROM ports WHERE node = ?").all(node) as unknown as Array<{
          port: number;
        }>
      ).map((r) => Number(r.port)),
    );
