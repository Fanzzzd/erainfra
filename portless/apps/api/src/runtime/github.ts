...
export async function installationToken(
  ...
  const res = await fetchImpl(`https://api.github.com/app/installations/${installationId}/access_tokens`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${jwt}`, Accept: 'application/vnd.github+json', 'User-Agent': 'portless' },
  });
...
export function verifyWebhook(secret: string, rawBody: Buffer | string, signature: string | undefined): boolean {
...
export function parsePush(body: unknown): PushEvent | null {
  const b = body as { ref?: string; after?: string; repository?: { full_name?: string }; installation?: { id?: number }; deleted?: boolean };
  const repo = b?.repository?.full_name;
  const ref = b?.ref;
  if (typeof repo !== 'string' || typeof ref !== 'string' || !ref.startsWith('refs/heads/') || b?.deleted) return null;
  return { repo, branch: ref.slice('refs/heads/'.length), sha: typeof b.after === 'string' ? b.after : '', installationId: b?.installation?.id };
}
...
export function githubAppConfig(): GithubAppConfig {
  ...
  if (!privateKey && process.env.PORTLESS_GH_APP_KEY_FILE) {
    try { privateKey = readFileSync(process.env.PORTLESS_GH_APP_KEY_FILE, 'utf8'); } catch { /* not present */ }
  }
