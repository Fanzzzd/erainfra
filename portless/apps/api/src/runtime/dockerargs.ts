...
function validatePublish(value: string): string | null {
  ...
  if (parts.length !== 2 && parts.length !== 3) return 'publish must be hostPort:containerPort or 127.0.0.1:hostPort:containerPort';
  if (parts.length === 3 && parts[0] !== '127.0.0.1') return 'explicit publish address must be 127.0.0.1';
...
function validateVolume(value: string): string | null {
  if (value.toLowerCase().includes('docker.sock')) return 'container-runtime sockets are not mountable';
  const parts = value.split(':');
  if (parts.length < 2 || parts.length > 3 || !VOLUME_NAME.test(parts[0])) return 'only named-volume mounts are allowed';
  const target = parts[1];
  if (!target.startsWith('/') || target === '/' || target.includes('/../') || target.endsWith('/..') || /[\0\n\r]/.test(target)) return 'invalid container mount target';
...
function validateEnv(value: string): string | null {
  ...
  if (equal < 1 || !ENV_NAME.test(value.slice(0, equal)) || /[\0\n\r]/.test(value)) return 'environment values must be KEY=value';
  return null;
}
