...
export class RouteStore {
  ...
  set(app: string, dep: Deployment): void {
    this.d
      .prepare(
        "INSERT INTO routes (app, node, image, port) VALUES (?, ?, ?, ?) ON CONFLICT(app) DO UPDATE SET node = excluded.node, image = excluded.image, port = excluded.port",
      )
  ...
  node(app: string): string | undefined {
    return (
      this.d.prepare("SELECT node FROM routes WHERE app = ?").get(app) as
        | { node: string }
        | undefined
    )?.node;
  }
  ...
  list(): Array<{ app: string } & Deployment> {
    return (this.d.prepare("SELECT * FROM routes ORDER BY app").all() as unknown as Row[]).map(
      (r) => ({ app: r.app, node: r.node, image: r.image, port: Number(r.port) }),
    );
  }
}
