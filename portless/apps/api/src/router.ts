...
const agentOperationSchema = z
  .object({
    ...
    args: z
      ...
      .refine((args) => Object.keys(args).length <= 16, "at most 16 operation arguments"),
  })
  .strict();
...
function recordOp(
  ...
  entry: { action: string; target: string; outcome: "success" | "failure"; dryRun?: boolean },
) {
...
function requireDurableAudit(
  ...
  if (!ctx.audit.lastWriteDurable()) {
    throw new TRPCError({
      ...
      message: "refusing: audit trail is not durable, cannot safely record this action",
    });
  }
}
...
export const appRouter = router({
    ...
    deploy: requirePermission("app.deploy")
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "confirm:true required to deploy to an agent",
          });
        ...
        try {
          ...
          const reply = await agentGateway.send(
            input.agentId,
            {
              cmd: "deploy",
              image: input.image,
              name: input.name,
              args: input.args,
              env: secretStore.get(input.name),
              port: input.port,
            },
            180_000,
          );
          if (reply.ok && input.port)
            routeStore.set(input.name, {
              node: input.agentId,
              image: input.image,
              port: input.port,
            }); // ingress + failover record
          const op = recordOp(ctx, {
            action: "agents.deploy",
            target: `${input.agentId}:${input.name}`,
    ...
    deployApp: requirePermission("app.deploy")
          ...
          services: z
            .array(
              z.object({
                name: z
                  .string()
                  .regex(/^[a-z0-9][a-z0-9-]{0,62}$/, "lowercase alphanumeric + dashes"),
                image: z.string().min(1).max(512),
                args: z.array(z.string().max(256)).max(64).default([]),
                ...
                route: z
                  ...
                  .optional(), // exposed service: its Args must publish `port`
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "confirm:true required to deploy an app to an agent",
          });
        const names = input.services.map((s) => s.name);
        if (new Set(names).size !== names.length)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "service names must be unique within the app",
          });
        const routes = input.services.filter((s) => s.route).map((s) => s.route!);
        if (new Set(routes).size !== routes.length)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "service routes must be unique within the app",
          });
        for (const s of input.services)
          if (s.route && !s.port)
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: `service "${s.name}" has a route but no port`,
            });
        for (const s of input.services) {
          ...
          if (dockerError)
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: `service "${s.name}": ${dockerError}`,
            });
        }
        ...
        for (const n of new Set(placed.map((s) => s.node)))
          if (!connected.has(n))
            throw new TRPCError({ code: "BAD_REQUEST", message: `node "${n}" is not connected` });
        requireDurableAudit(
          ctx,
          "agents.deployApp",
          `${input.app}: ${input.services.length} svc across ${new Set(placed.map((s) => s.node)).size} node(s)`,
        );
        try {
          const byNode = new Map<string, typeof placed>();
          for (const s of placed)
            (byNode.get(s.node) ?? byNode.set(s.node, []).get(s.node)!).push(s);
          ...
          for (const [node, svcs] of byNode) {
            const services = svcs.map((s) => ({
              ...
              port: s.port,
              route: s.route,
              env: { ...secretStore.get(input.app), ...secretStore.get(`${input.app}-${s.name}`) },
            }));
            const reply = await agentGateway.send(
              node,
              { cmd: "deployApp", app: input.app, services },
              300_000,
            );
          ...
          if (ok) {
            appStore.set(input.app, {
              node: input.agentId,
              services: placed.map((s) => ({
                name: s.name,
                image: s.image,
                args: s.args,
                port: s.port,
            ...
            for (const s of placed)
              if (s.route && s.port)
                routeStore.set(s.route, { node: s.node, image: s.image, port: s.port });
          }
          const op = recordOp(ctx, {
            action: "agents.deployApp",
            target: `${input.app}`,
    ...
    linkService: requirePermission("app.deploy")
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "confirm:true required to link a service across the mesh",
          });
        const connected = new Set(agentGateway.list().map((a) => a.id));
        for (const n of [input.provider, input.consumer])
          if (!connected.has(n))
            throw new TRPCError({ code: "BAD_REQUEST", message: `node "${n}" is not connected` });
        requireDurableAudit(
          ctx,
          "agents.linkService",
          `${input.name}: ${input.provider}:${input.providerPort} → ${input.consumer}:${input.localPort}`,
        );
        try {
          const share = await agentGateway.send(
            input.provider,
            { cmd: "meshShare", name: input.name, port: input.providerPort },
            60_000,
          );
          if (!share.ok || !share.output)
            throw new Error(`provider share failed: ${share.error ?? "no ticket returned"}`);
          // The ticket is a capability to reach the service — kept hub-internal, never returned to the caller.
          const conn = await agentGateway.send(
            input.consumer,
            {
              cmd: "meshConnect",
              name: input.name,
              ticket: share.output.trim(),
          ...
          const op = recordOp(ctx, {
            action: "agents.linkService",
            target: input.name,
            outcome: "success",
          });
          return {
            ok: true,
            address: `host.docker.internal:${input.localPort}`,
            localAddress: (conn.output ?? "").trim(),
            ...op,
          };
        } catch (e) {
    ...
    run: requirePermission("agent.run")
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "confirm:true required to run a command on an agent",
          });
        requireDurableAudit(ctx, "agents.run", `${input.agentId}: ${input.operation.name}`);
        try {
          const reply = await agentGateway.send(input.agentId, {
            cmd: "operate",
  ...
  mesh: router({
    list: requirePermission("app.read").query(() => {
      ...
      return linkStore
        .list()
        .map((l) => ({ ...l, online: connected.has(l.provider) && connected.has(l.consumer) }));
    }),
    ...
    link: requirePermission("app.deploy")
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "confirm:true required to create a mesh link",
          });
        const localPort = input.localPort ?? input.providerPort;
        const name =
          input.name ??
          `${input.provider}-${input.providerPort}-${input.consumer}`
            .toLowerCase()
            .replace(/[^a-z0-9-]/g, "-")
            .slice(0, 63);
        ...
        requireDurableAudit(
          ctx,
          "mesh.link",
          `${name}: ${input.provider}:${input.providerPort} → ${input.consumer}:${localPort}`,
        );
        try {
          ...
          return {
            ...
            containerAddress: `host.docker.internal:${localPort}`,
            ...op,
          };
        } catch (e) {
    ...
    unlink: requirePermission("app.deploy")
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "confirm:true required to remove a mesh link",
          });
        const link = linkStore.get(input.name);
        if (!link)
          throw new TRPCError({ code: "NOT_FOUND", message: `no such link: ${input.name}` });
        ...
        await agentGateway
          .send(link.provider, { cmd: "meshDrop", name: link.name }, 30_000)
          .catch(() => {});
        await agentGateway
          .send(link.consumer, { cmd: "meshDrop", name: link.name }, 30_000)
          .catch(() => {});
  ...
  git: router({
    ...
    unbind: requirePermission("app.deploy")
      ...
      .mutation(({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({ code: "BAD_REQUEST", message: "confirm:true required to unbind" });
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!hubBase)
          throw new TRPCError({
            ...
            message: "set PORTLESS_HUB_BASE (the hub url your build agents can reach) to deploy",
          });
        ...
        try {
          ...
          const op = recordOp(ctx, {
            action: "git.deployNow",
            target: `${binding.repo}:${binding.name}`,
            outcome: "success",
          });
    ...
    set: requirePermission("app.deploy")
      .input(
        z.object({
          ...
          vars: z.record(z.string().min(1).max(256), z.string().max(32768)),
        }),
      )
      .mutation(({ ctx, input }) => {
        const keys = Object.keys(input.vars);
        if (keys.length === 0)
          throw new TRPCError({ code: "BAD_REQUEST", message: "no vars given" });
        try {
    ...
    remove: requirePermission("app.deploy")
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "confirm:true required to remove an app",
          });
  ...
  backup: router({
    ...
    now: requirePermission("app.deploy")
      ...
      .mutation(async ({ ctx, input }) => {
        if (!input.confirm)
          throw new TRPCError({ code: "BAD_REQUEST", message: "confirm:true required to back up" });
        const cfg = backupConfig();
        if (!cfg)
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "backup not configured — set PORTLESS_BACKUP_S3_ENDPOINT/_BUCKET/_ACCESS_KEY/_SECRET_KEY",
          });
    ...
    deploy: requirePermission("app.deploy")
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!hubBase)
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message: "set PORTLESS_HUB_BASE (the hub url your build nodes can reach) to deploy",
          });
  ...
  apps: router({
    ...
    list: requirePermission("app.read").query(() => {
      ...
      const groupedRoutes = new Set(
        grouped.flatMap((g) => g.services.map((s) => s.route)).filter(Boolean),
      );
          ...
          services: [
            {
              ...
              route: r.app,
              url: domain ? `https://${r.app}.${domain}` : null,
              online: dataGateway.isConnected(r.node),
            },
          ],
    ...
    status: requirePermission("app.read")
      ...
      .query(({ input }) => {
        ...
        if (!d)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "no such deploy (hub restarted? redeploy)",
          });
        return d;
      }),
    ...
    logs: requirePermission("app.read")
      ...
      .query(async ({ input }) => {
        ...
        if (group) {
          ...
          if (!svc)
            throw new TRPCError({ code: "NOT_FOUND", message: `no such service in ${input.app}` });
        ...
        const reply = await agentGateway.send(
          node,
          {
            ...
            operation: {
              name: "container.logs",
              args: { name: container, lines: String(input.lines) },
            },
          },
          30_000,
        );
        if (!reply.ok)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: reply.error ?? "logs failed",
          });
  ...
  chats: router({
    ...
    remove: requirePermission("chat.write")
      .input(z.object({ id: z.string().min(1) }))
      .mutation(({ ctx, input }) => {
        if (!chatStore.remove(input.id))
          throw new TRPCError({ code: "NOT_FOUND", message: "no such chat session" });
  ...
  account: router({
    me: requirePermission("app.read").query(({ ctx }) => ({
      id: ctx.principal.id,
    ...
    changeEmail: requirePermission("app.read")
      ...
      .mutation(({ ctx, input }) => {
        ...
        if (!user)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "email change requires a user session (not an API token)",
          });
        ...
        const op = recordOp(ctx, {
          ...
          target: `${user.email} -> ${r.user.email}`,
          outcome: "success",
        });
    ...
    changePassword: requirePermission("app.read")
      ...
      .mutation(({ ctx, input }) => {
        ...
        if (!user)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "password change requires a user session (not an API token)",
          });
    ...
    sessions: router({
      list: requirePermission("app.read").query(({ ctx }) =>
        sessionStore.listForUser(ctx.principal.id),
      ),
      revoke: requirePermission("app.read")
        ...
        .mutation(({ ctx, input }) => {
          ...
          if (!mine || !sessionStore.revokeById(input.id))
    ...
    tokens: router({
      ...
      revoke: requirePermission("account.admin")
        .input(z.object({ id: z.string().min(1) }))
        .mutation(({ ctx, input }) => {
          if (!apiTokenStore.revoke(input.id))
      ...
      remove: requirePermission("account.admin")
        ...
        .mutation(({ ctx, input }) => {
          if (!input.confirm)
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "confirm:true required to remove a user",
            });
          if (input.id === ctx.principal.id)
