...
test("source upload round-trips and is auth-gated", async () => {
  ...
  try {
    ...
    assert.equal(
      (await app.inject({ method: "GET", url: `/builds/${buildId}/source.tgz` })).statusCode,
      401,
    );
    assert.equal(
      (await app.inject({ method: "GET", url: "/builds/..%2f..%2fetc/source.tgz", headers: AUTH }))
        .statusCode,
      400,
    );
    assert.equal(
      (
        await app.inject({
          method: "GET",
          url: "/builds/00000000-0000-0000-0000-000000000000/source.tgz",
          headers: AUTH,
        })
      ).statusCode,
      404,
