...
function fakeSocket() {
  return { sent: [] as string[], closed: false, send(d: string) { this.sent.push(d); }, close() { this.closed = true; } };
}
...
test('DataGateway.onClose rejects only the disconnecting node\'s in-flight requests (not every node)', async () => {
  ...
  const reqA = gw.proxy('A', 'appA', { method: 'GET', path: '/', headers: {}, body: Buffer.alloc(0) });
  const reqB = gw.proxy('B', 'appB', { method: 'GET', path: '/', headers: {}, body: Buffer.alloc(0) });
