...
test('parsePush extracts repo/branch/sha and skips non-branch events', () => {
  const ev = parsePush({ ref: 'refs/heads/main', after: 'abc123', repository: { full_name: 'o/r' }, installation: { id: 99 } });
  ...
  assert.equal(parsePush({ ref: 'refs/heads/main', deleted: true, repository: { full_name: 'o/r' } }), null); // branch delete
