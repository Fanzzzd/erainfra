...
const meta = (id: string, over: Record<string, unknown> = {}) => ({
  ...
  host: "mac",
  project: "/Users/me/code/portless",
  title: "fix the tunnel",
  ...
  s.replaceSession(meta("s1"), [
    {
      ...
      text: "how do I configure the cloudflare tunnel?",
    },
    {
      seq: 1,
      role: "assistant",
      model: "claude-fable-5",
      text: "Use cloudflared with a named tunnel and a config file.",
    },
  ]);
  s.replaceSession(
    meta("s2", { source: "codex", project: "/Users/me/code/blog", title: "blog css" }),
    [
      { seq: 0, role: "user", text: "center a div with css grid" },
      {
        ...
        role: "assistant",
        model: "gpt-5.5",
        text: "place-items: center on the grid container.",
      },
    ],
  );
...
test("http: chat endpoints are walled off from operator/viewer tokens", async () => {
  ...
  try {
    const ingest = (token: string) =>
      app.inject({
        ...
        payload: JSON.stringify({
          session: meta("h1"),
          messages: [{ seq: 0, role: "user", text: "secret sauce recipe" }],
        }),
      });
    ...
    const search = (token: string) =>
      app.inject({
        method: "GET",
        url: `/trpc/chats.search?input=${encodeURIComponent(JSON.stringify({ q: "secret sauce" }))}`,
    ...
    const hits = (ok.json() as { result: { data: Array<{ session: { id: string } }> } }).result
      .data;
