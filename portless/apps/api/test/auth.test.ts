...
test('user store: create/dup/verify, last-owner protection', () => {
  ...
  assert.ok(r.ok);
  assert.equal(s.create({ email: 'a@example.com', password: 'hunter22', roles: ['viewer'] }).ok, false); // dup, case-insensitive
  assert.equal(s.create({ email: 'b@example.com', password: 'short', roles: ['viewer'] }).ok, false); // weak pw
  ...
  if (r.ok) assert.match(s.remove(r.user.id).ok ? '' : (s.remove(r.user.id) as { error: string }).error ?? '', /owner|/);
...
test('http flow: setup-once → login (cookie) → tRPC → cli-token → logout', async () => {
  ...
  try {
    ...
    r = await app.inject({ method: 'POST', url: '/auth/setup', payload: { email: 'not-an-email', password: 'longenough' } });
    assert.equal(r.statusCode, 400);
    r = await app.inject({ method: 'POST', url: '/auth/setup', payload: { email: 'me@example.com', password: 'super-secret-1', name: 'Me' } });
    ...
    // setup is one-time
    r = await app.inject({ method: 'POST', url: '/auth/setup', payload: { email: 'evil@example.com', password: 'super-secret-2' } });
    ...
    r = await app.inject({ method: 'POST', url: '/auth/login', payload: { email: 'me@example.com', password: 'nope-nope-nope' } });
    ...
    for (let i = 0; i < 5; i++) {
      await app.inject({ method: 'POST', url: '/auth/login', payload: { email: 'brute@example.com', password: `guess-${i}` } });
    }
    r = await app.inject({ method: 'POST', url: '/auth/login', payload: { email: 'brute@example.com', password: 'guess-final' } });
