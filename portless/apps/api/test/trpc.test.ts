...
test("viewer cannot deploy and the denial is audited", async () => {
  ...
  await assert.rejects(
    () => call.agents.deploy({ agentId: "n1", image: "nginx", name: "demo", confirm: true }),
    /permission/,
  );
...
test("agents.deploy requires confirm:true before touching any agent", async () => {
  ...
  await assert.rejects(
    () => call.agents.deploy({ agentId: "n1", image: "nginx", name: "demo" }),
    /confirm/,
  );
});
...
test("agents.deploy preserves existing safe publish, env, named-volume, and mesh args", async (t) => {
  ...
  const args = [
    ...
    "8080:80",
    "-v",
    "demo-data:/var/lib/data",
    "--add-host=host.docker.internal:host-gateway",
  ];
  await caller(operator).call.agents.deploy({
    agentId: "n1",
    image: "busybox",
    name: "demo",
    args,
...
test("agents.run forwards a typed operation instead of argv", async (t) => {
  ...
  assert.deepEqual(sent, [
    { agentId: "n1", command: { cmd: "operate", operation: { name: "disk.usage", args: {} } } },
  ]);
});
...
test("agents.deployApp is RBAC-gated and validates the service set before touching any agent", async () => {
  ...
  await assert.rejects(
    () =>
      caller(viewer).call.agents.deployApp({
        agentId: "n1",
        app: "flight",
        services: svc,
        confirm: true,
  ...
  // confirm gate
  await assert.rejects(
    () => call.agents.deployApp({ agentId: "n1", app: "flight", services: svc }),
    /confirm:true required/,
  );
  // duplicate service names
  await assert.rejects(
    () =>
      call.agents.deployApp({
        agentId: "n1",
        app: "flight",
        services: [
          { name: "web", image: "a" },
  ...
  await assert.rejects(
    () =>
      call.agents.deployApp({
        agentId: "n1",
        app: "flight",
        services: [{ name: "web", image: "a", route: "flight" }],
        confirm: true,
      }),
    /route but no port/,
  );
...
test("agents.linkService is RBAC-gated, confirm-gated, and checks both nodes are connected", async () => {
  ...
  await assert.rejects(
    () => caller(viewer).call.agents.linkService({ ...args, confirm: true }),
    /missing permission/,
  );
  const { call } = caller(owner);
