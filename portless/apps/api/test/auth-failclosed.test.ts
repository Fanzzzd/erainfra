...
test("dev tokens work outside production (local dev default)", () => {
  withEnv(
    { NODE_ENV: undefined, PORTLESS_DEV_AUTH: undefined, PORTLESS_DEV_TOKENS: undefined },
    () => {
...
test("production fails closed: bundled dev tokens are refused", () => {
  withEnv(
    { NODE_ENV: "production", PORTLESS_DEV_AUTH: undefined, PORTLESS_DEV_TOKENS: undefined },
    () => {
...
test("production opt-in (PORTLESS_DEV_AUTH=1) re-enables dev tokens", () => {
  withEnv(
    { NODE_ENV: "production", PORTLESS_DEV_AUTH: "1", PORTLESS_DEV_TOKENS: undefined },
    () => {
      assert.equal(defaultTokenStore().resolve("owner-dev-token")?.roles[0], "owner");
    },
  );
...
test("explicit PORTLESS_DEV_TOKENS overrides everything", () => {
  withEnv(
    {
      NODE_ENV: "production",
      PORTLESS_DEV_TOKENS: JSON.stringify({
        "real-tok": { id: "u1", name: "Real", roles: ["admin"] },
      }),
    },
    () => {
