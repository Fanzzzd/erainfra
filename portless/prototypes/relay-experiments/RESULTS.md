...
- ~90 ms on a _loopback_ test ⇒ it **fell back to a remote relay** (a direct same-machine path
...
_did_ get a working public trycloudflare fetch once, so the CF path itself is sound.
