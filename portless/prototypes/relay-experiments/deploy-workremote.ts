...
// Run: node --experimental-strip-types prototypes/relay-experiments/deploy-workremote.ts
...
const PORT = 8787, APP = 'pl-demo', HOST_PORT = 18055; // registry:2 serves /v2/ on container 5000
...
try {
  ...
  for (let i = 0; i < 120 && !(await caller.agents.list()).some((a) => a.id === 'wr-node'); i++) await sleep(500);
  if (!(await caller.agents.list()).some((a) => a.id === 'wr-node')) throw new Error('wr-node never connected (control)');
  ...
  const r = await caller.agents.deploy({ agentId: 'wr-node', image: 'registry:2.8.2', name: APP, args: ['-p', `${HOST_PORT}:5000`], port: HOST_PORT, confirm: true });
