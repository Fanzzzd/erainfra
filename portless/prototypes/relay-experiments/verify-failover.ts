...
const PORT = 8797, APP = 'failover-app', APP_PORT = 8095, GRACE = 1500;
...
writeFileSync(join(src, 'index.js'), "const http=require('http');const p=process.env.PORT||3000;http.createServer((_,r)=>r.end('alive')).listen(p);");
...
const spawnAgent = (name: string): ChildProcess => spawn('deploy/bin/portless-agent-darwin-arm64',
  ['connect', '--hub', `ws://127.0.0.1:${PORT}/agent`, '--token', 'owner-dev-token', '--name', name], { stdio: ['ignore', 'ignore', 'ignore'] });
...
try {
  ...
  for (let i = 0; i < 40 && !(['nodeA', 'nodeB'].every((n) => dataGateway.isConnected(n))); i++) await sleep(250);
  if (!['nodeA', 'nodeB'].every((n) => dataGateway.isConnected(n))) throw new Error('both nodes never connected');
  ...
  const r = await caller.upload.deploy({ buildId: up.json().buildId, name: APP, port: APP_PORT, buildNode: 'nodeA', deployNode: 'nodeA', confirm: true });
  ...
  let res;
  for (let i = 0; i < 30; i++) { res = await get(`${APP}.apps.local`, '/'); if (res.status === 200) break; await sleep(500); }
  ...
  if (routeStore.node(APP) !== 'nodeB') throw new Error('route did not fail over to nodeB (still ' + routeStore.node(APP) + ')');
  console.log('✅ auto-failover flipped the route nodeA → nodeB');

  // Traffic must follow the route with no manual step.
  let after;
  for (let i = 0; i < 30; i++) { after = await get(`${APP}.apps.local`, '/'); if (after.status === 200) break; await sleep(500); }
  if (after?.body !== 'alive') throw new Error('app not serving after failover: ' + JSON.stringify(after));
  console.log('✅ traffic follows: <app>.apps.local still 200 after the node died (served via nodeB)');
  failed = false;
} catch (e) {
  console.error('FAIL:', (e as Error).message);
}
cleanup();
nodeA.kill('SIGKILL'); nodeB.kill('SIGKILL');
await app.close();
