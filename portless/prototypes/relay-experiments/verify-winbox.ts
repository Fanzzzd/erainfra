...
for (;;) {
  try {
    ...
    if (a && !seen.has(a.connectedAt as string)) {
      ...
      const r = await caller.agents.run({ agentId: AGENT, operation: { name: 'system.hostname', args: {} }, confirm: true });
      console.log('--- output from the Windows box, via portless ---');
      console.log((r.output ?? r.error ?? '').trim());
      console.log(r.ok ? '✅ portless is controlling the Windows box through a typed operation\n' : '');
    }
  } catch (e) {
