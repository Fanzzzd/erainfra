...
const PORT = 8790, APP = 'meshdb', DB_HOSTPORT = 15500, LINK_PORT = 15600, IMAGE = 'registry:2.8.2';
...
try {
  ...
  for (let i = 0; i < 60 && !(['nodeA', 'nodeB'].every((n) => dataGateway.isConnected(n))); i++) await sleep(500);
  const ids = (await caller.agents.list()).map((a) => a.id);
  if (!['nodeA', 'nodeB'].every((n) => ids.includes(n))) throw new Error(`agents not online: ${ids.join(',')}`);
  console.log('✅ nodeA + nodeB online');

  // db on nodeB, internal (no route) — published only to nodeB's loopback for the mesh to share.
  console.log(`deploying db (${IMAGE}) on nodeB, published to its loopback :${DB_HOSTPORT} (no public route)…`);
  const d = await caller.agents.deployApp({ agentId: 'nodeB', app: APP, services: [{ name: 'db', image: IMAGE, node: 'nodeB', args: ['-p', `127.0.0.1:${DB_HOSTPORT}:5000`] }], confirm: true });
  ...
  const link = await caller.agents.linkService({ name: 'dblink', provider: 'nodeB', providerPort: DB_HOSTPORT, consumer: 'nodeA', localPort: LINK_PORT, confirm: true });
  if (!link.ok) throw new Error('linkService failed');
  console.log(`✅ mesh link up — db reachable on nodeA at ${link.localAddress} (containers: ${link.address})`);

  // 1) host-side: reach the remote db THROUGH the mesh (this port is only served by dumbpipe-connect)
  let res; for (let i = 0; i < 20; i++) { try { res = await rawGet(LINK_PORT, '/v2/'); if (res.status === 200) break; } catch {} await sleep(1000); }
  console.log(`GET 127.0.0.1:${LINK_PORT}/v2/ (via mesh → nodeB db) → ${res?.status} ${JSON.stringify(res?.body)}`);
...
try { execSync(`docker rm -f ${APP}-db`, { stdio: 'ignore' }); } catch {}
try { execSync(`docker network rm ${APP}-net`, { stdio: 'ignore' }); } catch {}
try { execSync(`pkill -f 'dumbpipe (listen|connect)-tcp'`, { stdio: 'ignore' }); } catch {}
