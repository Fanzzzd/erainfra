 ...
 */
async function octokitFor(githubInstallationId: number | undefined): Promise<Octokit> {
  if (githubInstallationId !== undefined) {
    ...
    const privateKey = privateKeyValue.includes("\\n")
      ...
      : privateKeyValue;
    const app = new App({ appId: Number(appIdValue), privateKey });
...
export const issueJit = internalAction({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    try {
      ...
      const octokit = await octokitFor(args.githubInstallationId);
  ...
  handler: async (ctx): Promise<{ deleted: number; failed: number }> => {
    ...
    for (const deletion of pending) {
      try {
        ...
        const octokit = await octokitFor(deletion.githubInstallationId);
