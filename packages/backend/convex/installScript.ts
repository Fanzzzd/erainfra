...
if [ -z "$NODE_BIN" ]; then
  ...
  if command -v shasum >/dev/null 2>&1; then
    ACTUAL_SHA=$(shasum -a 256 "$NODE_TARBALL" | cut -d ' ' -f 1)
  elif command -v sha256sum >/dev/null 2>&1; then
    ACTUAL_SHA=$(sha256sum "$NODE_TARBALL" | cut -d ' ' -f 1)
  else
...
printf '✅ Downloading the Runner Center agent.\n'
SOURCE_TARBALL="$TMP_DIR/runner-center.tar.gz"
curl -fsSL 'https://github.com/Fanzzzd/runner-center/archive/refs/heads/main.tar.gz' -o "$SOURCE_TARBALL"
mkdir -p "$TMP_DIR/agent"
tar -xzf "$SOURCE_TARBALL" -C "$TMP_DIR/agent" --strip-components=3 runner-center-main/apps/agent
rm -rf "$AGENT_DIR"
mv "$TMP_DIR/agent" "$AGENT_DIR"
if [ -f "$ENV_BACKUP" ]; then
  cp "$ENV_BACKUP" "$ENV_FILE"
  chmod 600 "$ENV_FILE"
fi
chmod +x "$AGENT_DIR"/provisioners/*.sh

printf '✅ Installing dependencies and building the agent.\n'
(
  cd "$AGENT_DIR"
  "$NPM_BIN" install --include=dev --no-audit --no-fund
  "$NPM_BIN" run build
)
...
' "$RESPONSE_FILE")
  ...
  CONVEX_URL=$(grep '^CONVEX_URL=' "$ENV_FILE" | cut -d= -f2-)
  MACHINE_TOKEN=$(grep '^MACHINE_TOKEN=' "$ENV_FILE" | cut -d= -f2-)
  OLD_NAME=$(grep '^MACHINE_NAME=' "$META_FILE" 2>/dev/null | cut -d= -f2- || true)
  if [ -n "$OLD_NAME" ]; then
...
UNIT
  ...
  PID_FILE="$RC_HOME/agent.pid"
  if [ -f "$PID_FILE" ]; then
    OLD_PID=$(grep -E '^[0-9]+$' "$PID_FILE" || true)
...
umask 077
printf 'SITE_URL=%s\nNODE_BIN=%s\nMACHINE_NAME=%s\nSERVICE_KIND=%s\n' "$SITE_URL" "$NODE_BIN" "$MACHINE_NAME" "$SERVICE_KIND" > "$META_FILE"
chmod 600 "$META_FILE"
...
printf '✅ Runner Center is connected. Dashboard: %s\n' "$SITE_URL"
`;
