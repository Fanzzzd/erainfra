...
function requireConfig() {
  ...
  if (!cfg.hub || !cfg.token)
    die(
      `not connected to a hub — run: portless login https://your-hub  (or set PORTLESS_HUB + PORTLESS_TOKEN)`,
    );
  return cfg;
}
...
function packSource(dir) {
  ...
  if (inGit) {
    ...
    if (dirty)
      console.error(c.yellow(`! uncommitted changes are NOT included (git archive deploys HEAD)`));
    return execFileSync("git", ["-C", dir, "archive", "--format=tar.gz", "HEAD"], {
...
async function deployDir(cfg, dir, { app, port, node, buildNode } = {}, onStage) {
  ...
  const name = (app ?? basename(target))
    .toLowerCase()
    .replace(/[^a-z0-9-]/g, "-")
    .replace(/^-+|-+$/g, "");
  if (!/^[a-z0-9][a-z0-9-]{0,62}$/.test(name))
    throw new Error(`cannot derive an app name from "${basename(target)}" — pass --app`);
...
function parseCodexFile(file) {
  ...
  for (const line of readFileSync(file, "utf8").split("\n")) {
    ...
    let text = (Array.isArray(p.content) ? p.content : [])
      ...
      .join("\n")
      .trim();
    if (p.role === "user" && (/^<[a-z_ -]+>/i.test(text) || text.startsWith("# AGENTS.md")))
...
async function syncChats(cfg, { verbose = false } = {}) {
  let state = {};
  try {
    state = JSON.parse(readFileSync(CHAT_STATE_PATH, "utf8"));
  } catch {
    /* first run */
  }
  const targets = [
  ...
  for (const t of targets) {
    ...
    for (const file of listJsonlFiles(t.root)) {
      ...
      try {
        ...
        if (verbose)
          console.log(
            `  ${parsed.session.source} ${parsed.session.id.slice(0, 8)} ${parsed.messages.length} msgs`,
          );
...
const commands = {
  async login({ flags, pos }) {
    ...
    if (!hub)
      die("usage: portless login https://portless.example.com [--token <token> | --email <email>]");
  ...
  async nodes({ flags }) {
    ...
    if (!nodes.length) {
      ...
      console.log(
        c.cyan(
          `  curl -fsSL ${cfg.hub}/agent.sh | sudo sh -s -- --token <node-token> --name <name>`,
        ),
      );
      return;
    }
    table(
      nodes.map((n) => [n.id, n.version ?? "-", c.green("online"), n.connectedAt]),
      ["NODE", "VERSION", "STATUS", "CONNECTED AT"],
    );
  },
  ...
  async deploy({ flags, pos }) {
    const cfg = requireConfig();
    const t0 = Date.now();
    const d = await deployDir(
      cfg,
      pos[0],
      { app: flags.app, port: flags.port, node: flags.node, buildNode: flags["build-node"] },
      (p) => stageLine(p.detail ?? p.stage),
    );
    stageDone();
  ...
  async logs({ flags, pos }) {
    ...
    const r = await query(cfg, "apps.logs", {
      app,
      ...(service ? { service } : {}),
      lines: Number(flags.lines ?? 100),
    });
  ...
  async env({ flags, pos }) {
    ...
    if (!action) {
      ...
      table(
        vars.map((v) => [v.key, c.dim(v.preview)]),
        ["KEY", "VALUE"],
      );
    } else if (action === "set") {
      const vars = {};
      ...
      if (!Object.keys(vars).length)
        die("usage: portless env <app> set KEY=value [KEY2=value2 ...]");
      await mutate(cfg, "env.set", { app, vars });
      ok(
        `set ${Object.keys(vars).join(", ")} — takes effect on the next deploy (portless redeploy ${app})`,
      );
  ...
  async repos({ flags }) {
    ...
    if (!list.length)
      return console.log(
        "no git bindings — bind one in the dashboard, or: portless bind owner/repo --app <name>",
      );
    table(
      list.map((b) => [
        `${b.repo}@${b.branch}`,
        ...
        b.lastStatus
          ? b.lastStatus.ok
            ? c.green(`ok ${b.lastStatus.sha.slice(0, 7)}`)
            : c.red(`failed: ${b.lastStatus.stage}`)
          : c.dim("never"),
      ]),
  ...
  async bind({ flags, pos }) {
    ...
    if (!repo || !/^[\w.-]+\/[\w.-]+$/.test(repo))
      die("usage: portless bind owner/repo [--branch main] [--app name] [--port 3000]");
    const name =
      flags.app ??
      repo
        .split("/")[1]
  ...
  async redeploy({ pos }) {
    ...
    if (!b)
      die(
        `no git binding named "${app}" (portless repos to list; for directories use portless deploy)`,
      );
    const t0 = Date.now();
  ...
  async backup({ flags, pos }) {
    ...
    if (action === "now") {
      ...
      if (!conf.configured)
        return console.log("backups not configured — set PORTLESS_BACKUP_S3_* on the hub");
      const list = await query(cfg, "backup.list");
      if (flags.json) return console.log(JSON.stringify(list, null, 2));
      if (!list.length)
        return console.log(`no backups yet in ${conf.bucket} — run: portless backup now`);
      table(
        list.map((b) => [b.key, String(b.size), b.lastModified]),
        ["KEY", "BYTES", "MODIFIED"],
      );
    } else {
  ...
  async open({ pos }) {
    ...
    const apps = await query(cfg, "apps.list");
    const url =
      apps.flatMap((a) => a.services).find((s) => s.url && (s.route === app || s.name === app))
        ?.url ?? apps.find((a) => a.app === app)?.services.find((s) => s.url)?.url;
  ...
  async chats({ flags, pos }) {
    ...
    if (sub === "sync") {
      ...
      ok(
        `synced ${r.sent} changed session(s) (${r.skipped} unchanged, ${r.empty} empty, ${r.failed} failed) — hub now has ${st.sessions} sessions / ${st.messages} messages`,
      );
    } else if (sub === "list") {
      const cfg = requireConfig();
      const sessions = await query(cfg, "chats.sessions", {
      ...
      if (!sessions.length)
        return console.log("no chat sessions on the hub yet — run: portless chats sync");
      table(
        sessions.map((x) => [
          x.id.slice(0, 8),
          x.source,
          String(x.messageCount),
          (x.project ?? "").split("/").slice(-2).join("/"),
          (x.title ?? "").slice(0, 60),
          (x.updatedAt ?? "").slice(0, 16),
        ]),
      ...
      for (const h of hits) {
        console.log(
          `${c.bold(h.session.id.slice(0, 8))} ${c.dim(`${h.session.source} · ${h.role} · ${(h.at ?? "").slice(0, 16)} · ${(h.session.project ?? "").split("/").pop() ?? ""}`)}`,
        );
      ...
      console.log(
        `${c.bold(session.title ?? session.id)} ${c.dim(`(${session.source} · ${session.project ?? ""} · ${messages.length} msgs)`)}\n`,
      );
      ...
      const parsed =
        file.includes(".codex") || basename(file).startsWith("rollout-")
          ? parseCodexFile(file)
          : parseClaudeFile(file);
...
`,
        ...
        console.log(
          c.cyan(
            `*/15 * * * * ${process.execPath} ${bin} chats sync >>/tmp/portless-chats-sync.log 2>&1`,
          ),
        );
      }
    } else {
  ...
  async link({ flags, pos }) {
    ...
    const m = /^([^:]+):(\d+)$/.exec(src ?? "");
    if (!m || !dst)
      die("usage: portless link <provider-node>:<port> <consumer-node>[:<local-port>] [--name X]");
    ...
    ok(
      `${c.bold(consumer)} can now reach ${c.bold(src)} at ${c.cyan(r.address)} (containers: ${r.containerAddress}) — link "${r.name}", auto-healed`,
    );
  },
  ...
  async links({ flags }) {
    ...
    if (flags.json) return console.log(JSON.stringify(list, null, 2));
    if (!list.length)
      return console.log("no mesh links — create one: portless link <node>:<port> <node>");
    table(
      list.map((l) => [
        l.name,
        `${l.provider}:${l.providerPort}`,
        `${l.consumer} @127.0.0.1:${l.localPort}`,
        l.online ? c.green("online") : c.red("node offline"),
        c.dim(l.createdBy ?? "-"),
      ]),
...
const MCP_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        port: {
          type: "number",
          description: "container port — ONLY for projects without portless.yaml",
        },
  ...
  },
  {
    ...
    description:
      "Full-text search the archived Claude Code / Codex conversation history (all machines, all projects). Returns matching messages with snippets and session ids.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    handler: async (cfg, args) =>
      query(cfg, "chats.search", { q: args.query, limit: args.limit ?? 15 }),
  },
  {
    name: "portless_chats_get",
    description:
      "Fetch one archived AI conversation (all its user/assistant messages) by session id.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Set env vars / secrets on an app (encrypted at rest; applied on the next deploy).",
    inputSchema: {
      type: "object",
      properties: {
        app: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Back up the hub's control-plane state (routes, secrets, bindings, audit) to the configured S3.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
...
async function serveMcp() {
  ...
  for await (const line of rl) {
    ...
    const replyErr = (message, code = -32000) =>
      id !== undefined && respond({ jsonrpc: "2.0", id, error: { code, message } });
    try {
...
const name =
  { "-h": "help", "--help": "help", "-v": "version", "--version": "version" }[cmd] ?? cmd ?? "help";
