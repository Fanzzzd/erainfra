...
// Run: node --experimental-strip-types prototypes/relay-experiments/verify-install.ts
...
const cleanup = () => { try { execSync(`docker rm -f ${NODE}`, { stdio: 'ignore' }); } catch { /* gone */ } };
...
const node = spawn('docker', [
  'run', '--rm', '--name', NODE, '--add-host=host.docker.internal:host-gateway',
  'alpine', 'sh', '-c', oneLiner,
...
try {
  ...
  try { console.error(execSync(`docker logs ${NODE} 2>&1 | tail -20`).toString()); } catch { /* no logs */ }
}
cleanup();
node.kill('SIGKILL');
