...
// Run: node --experimental-strip-types prototypes/relay-experiments/verify-deps.ts
...
const PORT = 8795, APP = 'deps-app', APP_PORT = 8092;
...
writeFileSync(join(src, 'package.json'), JSON.stringify({
  name: 'deps-app', version: '1.0.0', scripts: { start: 'node index.js' },
...
try {
  ...
  console.log(`uploaded app (deps: mathjs, dayjs) → ${buildId.slice(0, 8)}; building (nixpacks runs npm install)…`);
  ...
  if (j.hypot === 5 && j.days === 365) console.log('✅ both libraries installed + computed correctly after deploy — zero extra config');
