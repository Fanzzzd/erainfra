...
// the caller. Run: node --experimental-strip-types prototypes/relay-experiments/verify-freshbox.ts
...
const PORT = 8787, BOX = 'pl-freshbox', AGENT = 'freshbox', APP = 'registry-demo', HOST_PORT = 18056, IMAGE = 'registry:2';
...
let failed = true;
try {
  console.log('waiting for the fresh box to enroll itself (one command installs docker + agent + dials in)…');
  for (let i = 0; i < 240 && !(await caller.agents.list()).some((a) => a.id === AGENT); i++) await sleep(500);
  if (!(await caller.agents.list()).some((a) => a.id === AGENT)) throw new Error(`${AGENT} never connected (control) — the one command did not finish`);
  ...
  console.log(`✅ the fresh box enrolled itself: agent "${AGENT}" online (control + data) — docker + agent came up from one command`);
  ...
  const r = await caller.agents.deploy({ agentId: AGENT, image: IMAGE, name: APP, args: ['-p', `${HOST_PORT}:5000`], port: HOST_PORT, confirm: true });
  ...
  let res;
  for (let i = 0; i < 30; i++) { res = await get(`${APP}.apps.local`, '/v2/'); if (res.status === 200) break; await sleep(1000); }
  ...
  console.log('✅ reached the app on the fresh box over the data plane — first-run is fully working');
  failed = false;
} catch (e) {
