...
import { backupNow, listBackups, restoreFrom, type S3Config } from '../../apps/api/src/runtime/backup.ts';

const ACCESS = 'portlessroot', SECRET = 'portlesssecret123', BUCKET = 'portless-backups';
...
writeFileSync(join(stateDir, 'routes.json'), JSON.stringify({ web: { node: 'nodeA', image: 'reg/web:1', port: 8080 } }));
...
mkdirSync(join(stateDir, 'builds')); writeFileSync(join(stateDir, 'builds', 'big.tgz'), Buffer.alloc(5000)); // must be EXCLUDED
...
const cfg: S3Config = { endpoint: 'http://127.0.0.1:9000', bucket: BUCKET, accessKey: ACCESS, secretKey: SECRET, region: 'us-east-1', prefix: 'portless/' };

let failed = true;
try {
  for (let i = 0; i < 40; i++) { try { if ((await fetch('http://127.0.0.1:9000/minio/health/live')).ok) break; } catch { /* booting */ } await sleep(250); }
  ...
  if (existsSync(join(restoreDir, 'builds'))) throw new Error('builds/ should have been excluded from the backup');
  ...
  try {
    ...
    if (/403|SignatureDoesNotMatch|→ 403/.test((e as Error).message)) console.log('✅ bad SigV4 signature rejected (403) — signing is real');
    else throw e;
  }
  failed = false;
} catch (e) {
...
minio.kill('SIGKILL');
try { rmSync(dataDir, { recursive: true, force: true }); rmSync(stateDir, { recursive: true, force: true }); } catch { /* best effort */ }
