...
func (r ShellRunner) ReadSpec(src BuildSource) (string, error) {
	...
	// Ordered, not exclusive (ADR 0004 stage 1). This name lives in a CUSTOMER's repository, so it
	// is not ours to rewrite — every repo bound to a Hub today carries portless.yaml and must keep
