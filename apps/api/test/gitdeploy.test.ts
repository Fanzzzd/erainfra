...
function fakeGw(replies: Record<string, { ok: boolean; output?: string; error?: string }> = {}) {
  ...
  return {
    calls,
    list: () => [{ id: 'bn', version: '0', roles: [], connectedAt: '' }, { id: 'dn', version: '0', roles: [], connectedAt: '' }],
...
test('git deploy with no portless.yaml: implicit single service (spec → build → deployApp)', async () => {
  ...
  const { deployId, done } = startGitDeploy(b, 'abcdef1234567890', undefined, { registry: 'reg:5000', hubBase: 'http://hub' }, gw);
  assert.ok(deployId);
  const r = await done;
...
`;
  ...
  const { done } = startUploadDeploy(id, { app: 'up1' }, { registry: 'reg:5000', hubBase: 'http://hub' }, gw);
  const r = await done;
  assert.equal(r.ok, true, r.error ?? '');

  assert.equal(gw.calls[0].cmd.tarUrl, `http://hub/builds/${id}/source.tgz`);
  // only web has build: — exactly one build call
  assert.deepEqual(gw.calls.map((c) => c.cmd.cmd), ['spec', 'build', 'deployApp']);
...
test('build failure short-circuits (no deployApp) and reports the stage', async () => {
  ...
  assert.deepEqual(gw.calls.map((c) => c.cmd.cmd), ['spec', 'build']); // deployApp never attempted
});
