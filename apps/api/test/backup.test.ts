...
test('backupConfig is null unless all S3 settings are present', () => {
  ...
  for (const k of ['PORTLESS_BACKUP_S3_ENDPOINT', 'PORTLESS_BACKUP_S3_BUCKET', 'PORTLESS_BACKUP_S3_ACCESS_KEY', 'PORTLESS_BACKUP_S3_SECRET_KEY']) delete process.env[k];
  ...
  for (const k of ['PORTLESS_BACKUP_S3_ENDPOINT', 'PORTLESS_BACKUP_S3_BUCKET', 'PORTLESS_BACKUP_S3_ACCESS_KEY', 'PORTLESS_BACKUP_S3_SECRET_KEY']) if (!(k in saved)) delete process.env[k];
});
