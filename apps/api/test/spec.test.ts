...
`,
  ...
  assert.ok(parsed.ok);
  if (!parsed.ok) return;
  const { placed, links } = planDeploy(parsed.spec, { defaultNode: 'web-box', registry: 'reg', sha: 'abc', app: 'flight' }, alloc);
  ...
  const again = planDeploy(parsed.spec, { defaultNode: 'web-box', registry: 'reg', sha: 'abc', app: 'flight' }, alloc);
...
test('planDeploy: same-node needs use docker DNS, no links, no host port for the dependency', () => {
  ...
  assert.ok(parsed.ok);
  if (!parsed.ok) return;
  const { placed, links } = planDeploy(parsed.spec, { defaultNode: 'n1', registry: 'reg', sha: 'x', app: 'one' }, alloc);
