...
export interface S3Config { endpoint: string; bucket: string; accessKey: string; secretKey: string; region: string; prefix: string }
...
export function backupConfig(): S3Config | null {
  ...
  return { endpoint: endpoint.replace(/\/$/, ''), bucket, accessKey, secretKey, region: process.env.PORTLESS_BACKUP_S3_REGION ?? 'us-east-1', prefix: process.env.PORTLESS_BACKUP_S3_PREFIX ?? 'portless/' };
}
...
function signV4(cfg: S3Config, method: string, canonicalUri: string, query: string, body: Buffer, now: Date, extraHeaders: Record<string, string> = {}): Record<string, string> {
  const host = new URL(cfg.endpoint).host;
  const amzDate = now.toISOString().replace(/[:-]|\.\d{3}/g, ''); // YYYYMMDDTHHMMSSZ
  const dateStamp = amzDate.slice(0, 8);
  const payloadHash = sha256hex(body);
  const baseHeaders: Record<string, string> = { host, 'x-amz-content-sha256': payloadHash, 'x-amz-date': amzDate, ...extraHeaders };
  const signedKeys = Object.keys(baseHeaders).map((k) => k.toLowerCase()).sort();
  const canonicalHeaders = signedKeys.map((k) => `${k}:${String(baseHeaders[Object.keys(baseHeaders).find((h) => h.toLowerCase() === k)!]).trim()}\n`).join('');
...
export async function putObject(cfg: S3Config, key: string, body: Buffer, now: Date = new Date()): Promise<void> {
  const uri = `/${cfg.bucket}/${encodeKey(key)}`;
  const headers = signV4(cfg, 'PUT', uri, '', body, now, { 'content-type': 'application/gzip' });
  const res = await fetch(`${cfg.endpoint}${uri}`, { method: 'PUT', headers, body: new Uint8Array(body) });
...
export async function listBackups(cfg: S3Config, now: Date = new Date()): Promise<string[]> {
  ...
  return [...xml.matchAll(/<Key>([^<]+)<\/Key>/g)].map((m) => m[1]).sort().reverse();
}
...
export function packState(dir: string = stateDir()): Buffer {
  return execFileSync('tar', ['-czf', '-', '-C', dir, '--exclude=builds', '--exclude=./builds', '--exclude=*.log', '--exclude=*.tmp', '.'], { maxBuffer: 256 * 1024 * 1024 });
}
...
export async function restoreFrom(cfg: S3Config, key: string, dir: string = stateDir()): Promise<void> {
