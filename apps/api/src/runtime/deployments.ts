...
export class DeploymentStore {
  ...
  create(app: string): Deployment {
    const dep: Deployment = { id: randomUUID(), app, stage: 'queued', detail: 'queued', urls: [], startedAt: new Date().toISOString() };
    this.d.prepare('INSERT INTO deployments (id, app, stage, detail, urls, started_at) VALUES (?, ?, ?, ?, ?, ?)')
      .run(dep.id, dep.app, dep.stage, dep.detail, '[]', dep.startedAt);
    this.d.prepare(`DELETE FROM deployments WHERE rowid NOT IN (SELECT rowid FROM deployments ORDER BY rowid DESC LIMIT ${KEEP})`).run();
    return dep;
  }
  ...
  update(id: string, patch: Partial<Pick<Deployment, 'stage' | 'detail' | 'urls' | 'error'>>): void {
    ...
    const finished = patch.stage === 'done' || patch.stage === 'failed' ? new Date().toISOString() : cur.finishedAt ?? null;
    this.d.prepare('UPDATE deployments SET stage = ?, detail = ?, urls = ?, error = ?, finished_at = ? WHERE id = ?')
  ...
  list(limit = 20, app?: string): Deployment[] {
    const rows = app
      ? this.d.prepare('SELECT * FROM deployments WHERE app = ? ORDER BY rowid DESC LIMIT ?').all(app, limit)
  ...
  failStale(reason: string): number {
    return Number(this.d.prepare("UPDATE deployments SET stage = 'failed', error = ?, finished_at = ? WHERE stage NOT IN ('done', 'failed')")
