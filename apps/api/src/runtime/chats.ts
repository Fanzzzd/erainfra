...
export class ChatStore {
  ...
  replaceSession(meta: ChatSessionMeta, messages: ChatMessage[]): void {
    this.d.exec('BEGIN');
    try {
      this.d.prepare(`INSERT INTO chat_sessions (id, source, host, project, title, started_at, updated_at, message_count)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ...
        .run(meta.id, meta.source, meta.host, meta.project ?? null, meta.title ?? null, meta.startedAt ?? null, meta.updatedAt ?? null, messages.length);
      ...
      const insMsg = this.d.prepare('INSERT INTO chat_messages (session_id, seq, role, model, at, text) VALUES (?, ?, ?, ?, ?, ?)');
      const insFts = this.d.prepare('INSERT INTO chat_fts (text, session_id, seq) VALUES (?, ?, ?)');
  ...
  get(id: string): ChatSession | undefined {
    const r = this.d.prepare('SELECT * FROM chat_sessions WHERE id = ?').get(id) as SessionRow | undefined;
  ...
  listSessions(opts: { source?: string; project?: string; limit?: number } = {}): ChatSession[] {
    const cond: string[] = [];
    const params: string[] = [];
    if (opts.source) { cond.push('source = ?'); params.push(opts.source); }
    if (opts.project) { cond.push('project LIKE ?'); params.push(`%${opts.project}%`); }
    const where = cond.length ? `WHERE ${cond.join(' AND ')}` : '';
    const rows = this.d.prepare(`SELECT * FROM chat_sessions ${where} ORDER BY updated_at DESC LIMIT ?`).all(...params, opts.limit ?? 50);
    return (rows as unknown as SessionRow[]).map(toSession);
  }

  messages(sessionId: string): ChatMessage[] {
    const rows = this.d.prepare('SELECT seq, role, model, at, text FROM chat_messages WHERE session_id = ? ORDER BY seq').all(sessionId);
    return (rows as unknown as Array<{ seq: number; role: string; model: string | null; at: string | null; text: string }>)
      .map((r) => ({ seq: Number(r.seq), role: r.role as ChatMessage['role'], model: r.model ?? undefined, at: r.at ?? undefined, text: r.text }));
  }
  ...
  stats(): { sessions: number; messages: number } {
    return {
      sessions: Number((this.d.prepare('SELECT COUNT(*) n FROM chat_sessions').get() as { n: number }).n),
