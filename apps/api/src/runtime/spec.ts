...
const volumeRef = z.string().regex(/^[a-z0-9][a-z0-9-]{0,62}:\/[^\s:]+$/, 'volumes are "name:/container/path" (named volumes only)');
...
const serviceSchema = z
  .object({
    ...
    env: z.record(z.string().regex(envKey, 'a valid env var name'), z.string().max(32768)).optional(),
...
export function parseSpec(yamlText: string, fallbackApp: string): SpecResult {
  ...
  if (!parsed.success) {
    ...
    return { ok: false, error: `portless.yaml: ${first.path.join('.') || '(root)'}: ${first.message}` };
  }
  const app = parsed.data.app ?? fallbackApp;
  if (!LABEL.test(app)) return { ok: false, error: `app name "${app}" must be lowercase alphanumeric + dashes` };
  ...
  for (const name of names) {
    ...
    if (!!s.build === !!s.image) return { ok: false, error: `service "${name}": set exactly one of build / image` };
    if (s.build !== undefined && (s.build.startsWith('/') || s.build.split('/').includes('..'))) {
      return { ok: false, error: `service "${name}": build must be a relative dir inside the repo` };
    }
    ...
    if (route && !s.port) return { ok: false, error: `service "${name}": route requires port (what should the URL proxy to?)` };
    for (const need of s.needs ?? []) {
      if (!names.includes(need)) return { ok: false, error: `service "${name}": needs unknown service "${need}"` };
      if (need === name) return { ok: false, error: `service "${name}": cannot need itself` };
      if (!parsed.data.services[need].port) return { ok: false, error: `service "${need}": needs a port (it is a dependency of "${name}")` };
    }
    services.push({
      name,
      build: s.build,
  ...
  if (new Set(routes).size !== routes.length) return { ok: false, error: 'route labels must be unique within the app' };
...
export function implicitSpec(app: string, port: number): AppSpec {
  return { app, services: [{ name: 'web', build: '.', port, route: app, env: {}, volumes: [], needs: [] }] };
}
