...
export async function failoverNode(lostId: string, deps: FailoverDeps): Promise<FailoverResult[]> {
  ...
  const groupRoutes = new Set(
    groups.flatMap((g) => g.services.map((s) => s.route).filter((r): r is string => !!r)),
  );
  ...
  const healthy = deps.gateway
    .list()
    .map((a) => a.id)
    .filter((id) => id !== lostId);
  if (!healthy.length) {
    log(
      `node ${lostId} down with ${affected.length + groups.length} app(s) but no surviving node to take over`,
    );
    return [...affected.map((a) => a.app), ...groups.map((g) => g.app)].map((app) => ({
      app,
      from: lostId,
      to: "",
      ok: false,
      error: "no surviving node",
    }));
  }
  ...
  for (const app of affected) {
    ...
    try {
      const reply = await deps.gateway.send(
        to,
        {
          cmd: "deploy",
          image: app.image,
          name: app.app,
          args,
          env: deps.secrets.get(app.app),
          port: app.port,
        },
        180_000,
      );
      if (reply.ok) {
        deps.routes.set(app.app, { node: to, image: app.image, port: app.port });
        log(`failed over ${app.app}: ${lostId} → ${to}`);
  ...
  for (const g of groups) {
    ...
    const services = g.services.map((s) => {
      const env = {
        ...s.env,
        ...deps.secrets.get(g.app),
        ...deps.secrets.get(`${g.app}-${s.name}`),
      };
    ...
    try {
      const reply = await deps.gateway.send(
        to,
        { cmd: "deployApp", app: g.app, services },
        300_000,
      );
      if (reply.ok) {
        ...
        for (const s of g.services)
          if (s.route && s.port)
            deps.routes.set(s.route, { node: to, image: s.image, port: s.port });
...
export function installFailover(
  deps: FailoverDeps = {
