...
export class UserStore {
  ...
  list(): PublicUser[] {
    return (
      this.d.prepare("SELECT * FROM users ORDER BY created_at").all() as unknown as Row[]
    ).map((r) => publicView(toUser(r)));
  }
  ...
  create(input: {
    ...
    this.d
      .prepare(
        "INSERT INTO users (id, email, name, roles, password_hash, created_at) VALUES (?, ?, ?, ?, ?, ?)",
      )
      .run(
        user.id,
        user.email,
  ...
  setPassword(id: string, newPassword: string): { ok: true } | { ok: false; error: string } {
    if (newPassword.length < 8)
      return { ok: false, error: "password must be at least 8 characters" };
    const changed = this.d
      .prepare("UPDATE users SET password_hash = ? WHERE id = ?")
      .run(hashPassword(newPassword), id).changes;
  ...
  remove(id: string): { ok: true } | { ok: false; error: string } {
    ...
    if (u.roles.includes("owner")) {
      const owners = (
        this.d.prepare("SELECT roles FROM users").all() as unknown as Array<{ roles: string }>
