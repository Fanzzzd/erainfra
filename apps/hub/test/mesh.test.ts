  ...
  const gw = {
    ...
    send: async (to: string, cmd: any) => {
      ...
      if (cmd.cmd === "meshShare")
        return opts.failShare
          ? { ok: false, error: "share boom" }
          : { ok: true, output: "TICKET-abc\n" };
      if (cmd.cmd === "meshConnect")
        return opts.failConnect
          ? { ok: false, error: "connect boom" }
          : { ok: true, output: "127.0.0.1:9999" };
      return { ok: true };
    },
  };
...
test("establishLink refuses when either node is offline and surfaces agent errors", async () => {
  ...
  await assert.rejects(
    () => establishLink(LINK, fakeGw(["boxa", "boxb"], { failShare: true }).gw),
    /share boom/,
  );
  await assert.rejects(
    () => establishLink(LINK, fakeGw(["boxa", "boxb"], { failConnect: true }).gw),
    /connect boom/,
  );
});
