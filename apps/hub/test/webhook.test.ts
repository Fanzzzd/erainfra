...
test("github webhook verifies signature and routes events", async () => {
  ...
  try {
    ...
    assert.equal(
      (await post(app, push, { "x-github-event": "push", "x-hub-signature-256": "sha256=bad" }))
        .statusCode,
      401,
    );

    // valid ping → pong
    const ping = await post(app, "{}", {
      "x-github-event": "ping",
    ...
    const tagRes = await post(app, tag, {
      "x-github-event": "push",
      "x-hub-signature-256": sign(tag),
    });
