...
//   node --experimental-strip-types apps/hub/scripts/verify/verify-wslnode.ts
...
try {
  ...
  for (let i = 0; i < 1200 && !(await caller.agents.list()).some((a) => a.id === AGENT); i++)
    await sleep(500);
