...
//   node --experimental-strip-types apps/hub/scripts/verify/verify-multicontainer.ts
...
try {
  ...
  for (let i = 0; i < 60 && !(await caller.agents.list()).some((a) => a.id === AGENT); i++)
    await sleep(500);
  if (!(await caller.agents.list()).some((a) => a.id === AGENT))
    throw new Error("agent never connected (control)");
