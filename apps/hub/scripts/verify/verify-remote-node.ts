...
// Run: node --experimental-strip-types apps/hub/scripts/verify/verify-remote-node.ts
...
const node = spawn(
  ...
  [
    ...
    "-v",
    `${process.cwd()}/deploy/infra/bin/portless-agent-linux-arm64:/pl-agent:ro`, // built by build-agents.sh (Docker Desktop runs linux/arm64)
    "-v",
    ...
    "connect",
    "--hub",
    `ws://host.docker.internal:${PORT}/agent`,
    "--token",
    "owner-dev-token",
    "--name",
    "linux-node",
...
try {
  let info;
  for (let i = 0; i < 60; i++) {
    info = (await caller.agents.list()).find((a) => a.id === "linux-node");
    if (info) break;
    await sleep(500);
  }
  if (!info)
    throw new Error(
      "remote node never registered (check firewall / host.docker.internal reachability)",
    );
  ...
  console.log(
    `node hostname=${JSON.stringify((h.output ?? "").trim())}  (this Mac=${JSON.stringify(macHost)})`,
  );
  if (!h.ok || (h.output ?? "").trim() === macHost)
    throw new Error("exec did not run on a distinct node");
  ...
  const d = await caller.agents.deploy({
    agentId: "linux-node",
    image: IMAGE,
    name: APP,
    args: ["-p", `${APP_PORT}:3000`],
    confirm: true,
  });
  ...
  try {
    console.error(execSync(`docker logs ${NODE} 2>&1 | tail -15`).toString());
  } catch {
    /* no logs */
  }
  failed = true;
