...
// Run: node --experimental-strip-types apps/hub/scripts/verify/verify-dataplane.ts
...
writeFileSync(
  ...
  "const http=require('http');const p=process.env.PORT||3000;" +
    "http.createServer((req,res)=>{res.setHeader('x-app','" +
    APP +
    "');" +
  ...
  return new Promise((resolve, reject) => {
    const r = http.request(
      ...
      (res) => {
        ...
        res.on("data", (c) => (b += c));
        res.on("end", () =>
          resolve({ status: res.statusCode ?? 0, headers: res.headers, body: b }),
        );
      },
    );
    r.on("error", reject);
...
try {
  ...
  for (let i = 0; i < 30; i++) {
    try {
      res = await req(`${APP}.apps.local`, "/hello?x=1");
  ...
  if (health.status !== 200 || !health.body.includes("ok"))
    throw new Error("hub host should bypass proxy: " + health.status);
