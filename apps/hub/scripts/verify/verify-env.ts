...
// Run: node --experimental-strip-types apps/hub/scripts/verify/verify-env.ts
...
writeFileSync(
  join(src, "index.js"),
  "const http=require('http');const p=process.env.PORT||3000;http.createServer((_,r)=>r.end(process.env.GREETING||'unset')).listen(p);",
);
...
try {
  ...
  if (JSON.stringify(masked).includes("hello-from-secret"))
    throw new Error("value leaked in list!");
  ...
  if (body === "hello-from-secret")
    console.log("✅ secret reached the running container (injected as env)");
