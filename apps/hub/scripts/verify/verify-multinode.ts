...
//   node --experimental-strip-types apps/hub/scripts/verify/verify-multinode.ts
...
const PORT = 8789,
  APP = "mndemo",
  IMAGE = "nginx:alpine";
const AGENT_BIN = new URL("../../deploy/infra/bin/portless-agent-darwin-arm64", import.meta.url)
  .pathname;
...
try {
  ...
  for (
    let i = 0;
    i < 60 && !["nodeA", "nodeB"].every((n) => (caller as any) && dataGateway.isConnected(n));
    i++
  )
    await sleep(500);
  const ids = (await caller.agents.list()).map((a) => a.id);
  if (!["nodeA", "nodeB"].every((n) => ids.includes(n) && dataGateway.isConnected(n)))
    throw new Error(`both agents not online: ${ids.join(",")}`);
  ...
  if (r.results.length !== 2)
    throw new Error(`expected 2 per-node deploys, got ${r.results.length}`);
  console.log(
    `✅ deployApp fanned out to ${r.results.length} nodes: ${r.results.map((x) => x.node).join(", ")}`,
  );

  // routes resolve to the correct node
  const routes = await caller.routes.list();
  const web = routes.find((x) => x.app === "mn-web"),
    api = routes.find((x) => x.app === "mn-api");
  ...
  for (const [host, label] of [
    ["mn-web.apps.local", "web@nodeA"],
    ["mn-api.apps.local", "api@nodeB"],
  ] as const) {
    let res;
    for (let i = 0; i < 30; i++) {
      res = await get(host, "/");
    ...
    if (res?.status !== 200 || !/nginx/i.test(res.body))
      throw new Error(`reach failed for ${host}: ${res?.status}`);
  }
  ...
  console.log(
    `after nodeB down: mn-web→${stillWeb.status} (want 200), mn-api→${goneApi.status} (want non-200)`,
  );
  if (stillWeb.status !== 200 || goneApi.status === 200)
    throw new Error(`per-node routing not proven: web=${stillWeb.status} api=${goneApi.status}`);
...
try {
  execSync(`docker rm -f ${APP}-web ${APP}-api`, { stdio: "ignore" });
} catch {}
try {
